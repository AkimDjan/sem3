...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        ...
        fin >> N; fin.close();
        for (int i=1; i<size; i++) {Check_For_SUCCESS(MPI_Send(&N, 1, MPI_LONG_LONG, i, 1, MPI_COMM_WORLD));}
    } else {MPI_Status st; Check_For_SUCCESS(MPI_Recv(&N, 1, MPI_LONG_LONG, 0, 1, MPI_COMM_WORLD, &st));}
    ...
    if (rank == 0) {
        ...
        for (int i=1; i<size; i++) { 
            MPI_Status st;
            double received_sum;
            Check_For_SUCCESS(MPI_Recv(&received_sum, 1, MPI_DOUBLE, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, &st));
            pi += received_sum;
        }
    } else {Check_For_SUCCESS(MPI_Send(&local_sum, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD));}
    if (rank == 0) {
